...
namespace Karen
{
  OpenGlTexture2D::OpenGlTexture2D(const std::string& file_path, bool flip_v)
  {
    ...
    {
      KAREN_PROFILE_SCOPE("ImageLoader loadFromFile");
      status = m_image.loadFromFile(file_path, flip_v);
      KAREN_CORE_ASSERT_MSG(status, std::string("Failed to load Texture from file: ") + file_path);
    }
    uint32_t fmt = 0;
    ...
    m_data_format = fmt;
    KAREN_CORE_ASSERT_MSG(m_image.getColorChannels() > 2 && m_image.getColorChannels() < 5, "Unsupported Image format");
    {
      KAREN_PROFILE_SCOPE("Setting glTexParameters");
