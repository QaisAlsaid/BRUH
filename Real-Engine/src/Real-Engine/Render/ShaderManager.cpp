...
namespace Real
{
  ...
  void ShaderManager::LoadConfig(const std::string& p_config_file_path)
  {
    ...
    REAL_CORE_INFO("Started Loading Shaders from config");
    ...
    for (pugi::xml_node shader : list.children())
    {
      ...
      {
        is_path = false;
        REAL_CORE_ERROR("No Path for Shader number: {0} name: {1} it Won't be loaded", index, name);
      }
      if(name == "" || (!shader.child("Name")))
      {
        if(is_path)
        {
          ...
          REAL_CORE_WARN("No name specified for Shader Number: {0} Path: {1} , will be assigned the name : {2}", index, path, path_name);
          name = path_name;
        }
      }
      REAL_CORE_INFO("Shader number: {0} Name: {1} Path: {2}", index, name, path);
      if(is_path)
      {
      ...
      {
        REAL_CORE_WARN("No path specified for Shader number: {0} it won't be added", index);
      }
      REAL_CORE_INFO("NAME : {0}", name);
    }
  }

  void ShaderManager::Add(const std::string& p_name, const ARef<Shader>& p_shader)
  {
    REAL_CORE_ASSERT_MSG(m_shaders.find(p_name) != m_shaders.end(), std::string("Shader with Name: {0} already exist it won't be added"));
  ...
  void ShaderManager::Add(const std::string& p_path, const std::string& p_name) 
  {
    ...
    if(p_path == "")
    {
      REAL_CORE_ASSERT_MSG(false,"No Path specified Shader Won't be Added");
    }
    else if(p_name == "")
    {
      new_name = getNameFromPath(p_path);
      REAL_CORE_WARN("No Name specified for Shader Path: {0} it will be assigned the name: {1}", p_path, new_name); 
    }
    ARef<Shader> shader = Shader::create(p_path);
    REAL_CORE_ASSERT_MSG(m_shaders.find(new_name) != m_shaders.end(), std::string("Shader with Name: {0} already exist it wont be added" + new_name))
  ...
  ARef<Shader> ShaderManager::get(const std::string& p_name)
  {
    ...
    {
      REAL_CORE_ASSERT_MSG(false, std::string("Shader with name: ") + p_name + "Not found");
      return nullptr;
    }
    else
